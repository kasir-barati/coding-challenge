...
export function Projects() {
  ...
  const handlePurchaseClick = (id: string) => {
    ...
    const project = projectsPurchaseState.find((project) => project.id === id);
      ...
      <Stack rowGap={2}>
        ...
        <Grid container spacing={2}>
          {projects.map((project, index) => {
            const projectPurchaseState = projectsPurchaseState.find(
              (projectPurchaseState) => projectPurchaseState.id === project.id
            );
                ...
                <Card key={index}>
                    ...
                    >
                      ...
                      <Box sx={{ textAlign: "right" }}>
                        ...
                        <Typography variant="body2">
                          {convertProjectStateDate(project.start_date)}
                        </Typography>
                      </Box>
                    </Stack>
                  </CardContent>
                  <CardActions>
                    <Button
                      ...
                      variant="contained"
                      color={
                        projectPurchaseState?.isPurchased ? "error" : "primary"
                      }
                      onClick={() => handlePurchaseClick(project.id)}
                      sx={{ borderRadius: 100 }}
                    >
                      purchase
                    </Button>
